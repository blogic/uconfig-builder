<script>
  ...
  const ensurePaths = $derived(layout.filter((n) => n.objectSection).map((n) => n.objectSection))
  $effect(() => {
  ...
  function walk(o, path, create = false) {
    if (!path) return o
    let cur = o
  ...
  function parent_of(path) {
    ...
    return dot < 0 ? data : walk(data, path.slice(0, dot), true)
  }
